...
|---|---|
| `examples/encode_dataset.py` | Converts raw audio to pre-tokenized `.pt` files using a validated mel-spectrogram → VQ workaround (the public `heartlib` API has no audio tokenization path) |
| `examples/train_lora.py` | LoRA fine-tuning loop with mixed-dtype handling, gradient checkpointing, and multi-GPU support via `accelerate` |
| `examples/run_music_generation.py` | CLI inference wrapper with LoRA adapter support |
| `examples/prepare_lora_dataset.py` | End-to-end dataset prep: HDemucs source separation → HeartTranscriptor lyrics → Audio Flamingo tags |
...
[
  {
    "audio": "./data/processed/song1_vocals.wav",
    "lyrics": "verse one...",
    "tags": "pop, upbeat, female vocals, guitar"
  }
]
```
...
data/
  ...
  dataset.json                # manifest: audio path + lyrics + tags per clip
